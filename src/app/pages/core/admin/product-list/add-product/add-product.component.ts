import { Component, OnInit } from '@angular/core';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import { AddProductService } from './add-product.service';

export interface dropDownArray {
  id: string;
  name: string;
}

@Component({
  selector: 'app-add-product',
  templateUrl: './add-product.component.html',
  styleUrls: ['./add-product.component.scss'],
})
export class AddProductComponent implements OnInit {
  constructor(private addProductService: AddProductService) {}
  QuantityUnits: dropDownArray[] = [
    {
      id: '0',
      name: 'Kg',
    },
    {
      id: '1',
      name: 'grams',
    },
    {
      id: '2',
      name: 'piece',
    },
  ];

  categorys: dropDownArray[] = [
    {
      id: '0',
      name: 'sweets',
    },
    {
      id: '1',
      name: 'crispies',
    },
    {
      id: '2',
      name: 'chips',
    },
  ];

  onEditForm: boolean = false;
  productForm = new FormGroup<any>({
    product_name: new FormControl('', Validators.required),
    quantity: new FormControl('', Validators.required),
    unit: new FormControl('', Validators.required),
    priceinINR: new FormControl('', Validators.required),
    priceinAED: new FormControl('', Validators.required),
    priceinEUR: new FormControl('', Validators.required),
    priceinUSD: new FormControl('', Validators.required),
    // priceAUD: new FormControl(null, Validators.required),
    product_description: new FormControl('', Validators.required),
    shipping: new FormControl('', Validators.required),
    free_delivery: new FormControl('', Validators.required),
    cod: new FormControl('', Validators.required),
    reFund: new FormControl('', Validators.required),
    // coupon: new FormControl('', Validators.required),
    image: new FormControl(''),
    category: new FormControl('', Validators.required),
  });
  isLoder = false;
  ngOnInit(): void {}
  onSubmit() {
    // if (this.productForm.valid) {
      this.isLoder = true;
      this.addProductService.addProduct(this.productForm.value).subscribe({
        next: (res) => {
          if (res.result) {
            this.isLoder =false
          }else{
            this.isLoder = false;
            alert("Something is wrong")
          }
        },
      });
    // }
  }
  resetForm() {}
  onImageChange(event: any) {
    const file = (event.target as HTMLInputElement).files?.[0];
    this.productForm.patchValue({
      image: file,
    });
  }
  passUnitValue(selectedValue: any) {
    // Handle the selected value here
    this.productForm.patchValue({
      unit: selectedValue,
    });
  }
  passCategorysValue(selectedValue: any) {
    this.productForm.patchValue({
      category: selectedValue,
    });
  }
}
